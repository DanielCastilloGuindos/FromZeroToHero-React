...
export const Navegation = ({ data }) => {
	...
	return (
		<nav className={`fixed top-0 left-0 ${isActivatedMenu ? 'w-1/4 sm:w-full' : 'w-full'} flex shrink-0 justify-end items-center self-stretch gap-9 bg-white ${ borderClass } h-18 py-3 sm:px-20 z-20 transition-all duration-1000`}>

			<Hambuger handleClick={ handleActivateMenu } />

			<div className={`w-3/4 sm:w-auto h-screen sm:h-12 fixed sm:relative top-0 ${ menuClass } sm:left-0 pt-8 sm:pt-0 px-8 sm:px-0 flex flex-col sm:flex-row shrink-0 justify-start sm:justify-center items-start sm:items-center gap-10  bg-white sm:bg-transparent border-b border-l border-gray-400 sm:border-b-0 sm:border-l-0 [transition-property: left] duration-1000`}>
				<NavigationItem
					path='#nosotros'
					title='Nosotros' />
				<NavigationItem
